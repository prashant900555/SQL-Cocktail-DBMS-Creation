/* Practical Week 4 - A cocktail for SQL*/

/* Student No. - 20200126*/
/* Name - Prashant Wakchaure*/

/* Content from Practical Week 2 & Practical Week 3 to get the previous weeks' tables*/
...
/* Have a look at the costs table */
SELECT 
    *
FROM
    costs;

/* Using table joins, calculate the cost price of each cocktail in DB.*/
/*Explanation: The cost price of each cocktail will be the sum(quantity of ingredients * cost per unit(ml)) 
grouped by the cocktails */
SELECT 
...
/*List all cocktails in descending order of cost price to the maker.*/
/*Explanation: The cost price of each cocktail will be the sum(quantity of ingredients * cost per unit(ml)), 
grouped by the cocktails which we will order by and use DESC to display in descending order */
SELECT 
    ingredients.cocktail_name AS alcoholic_cocktails,
    ROUND(SUM(ingredients.quantity * costs.`cost_per_unit(euros)`),
            2) AS `total_cost(euros)`
FROM
    costs
        INNER JOIN
    ingredients ON ingredients.ingredient_name = costs.ingredient_name
GROUP BY cocktail_name ORDER BY `total_cost(euros)` DESC;

/* List all cocktails in descending order of total alcohol content. */
/*Explanation: The total alcohol content is calculated by sum((abv% of each ingredient * ingredients quantity) / sum(indregients quantity)) 
grouped by the cocktails and finally ordered by the total abv % of the cocktail in descending order */
SELECT 
    ingredients.cocktail_name AS alcoholic_cocktails,
...
GROUP BY cocktail_name ORDER BY `total_abv(%)` DESC;
