/* Practical Week 3 - A cocktail for SQL*/

/* Student No. - 20200126*/
/* Name - Prashant Wakchaure*/

/* Content from Practical Week 2*/
...
CREATE TABLE `ingredients` (
    ...
    quantity DOUBLE(10 , 2 ) DEFAULT 0,
...
/* We will look at the ingredients and the abv ratings table - SELECT*/
SELECT 
    *
FROM
    ingredients;
...
/* Use a table join to list all alcoholic ingredients of all cocktails in DB.
Explanation: To display all the alchoholic ingredients of all cocktails, we select the ingredient name, cocktail name, ingredient type, and the abv % of each ingredient which is of type Alcoholic.
*/
SELECT 
    ingredients.ingredient_name,
    ingredients.cocktail_name,
    abv_ratings.ingredient_type,
    abv_ratings.`abv(%)`
FROM
    ingredients
        INNER JOIN
    abv_ratings ON ingredients.ingredient_name = abv_ratings.ingredient_name
WHERE
    abv_ratings.ingredient_type = 'Alcoholic Drink';

/*Use a join to list all non-alcoholic ingredients of all cocktails in DB.
Explanation: To display all the non-alchoholic ingredients of all cocktails, we select the ingredient name, cocktail name, ingredient type, and the abv % of each ingredient which is of type Soft Drink.
*/
SELECT 
    ingredients.ingredient_name,
    ingredients.cocktail_name,
...
FROM
    ...
    abv_ratings ON ingredients.ingredient_name = abv_ratings.ingredient_name
WHERE
    abv_ratings.ingredient_type = 'Soft Drink';

/*List all alcoholic cocktails in DB.
Explanation: To display all the alchoholic cocktails, I sorted out the total alcoholic content of a particular cocktail from its ingredients' quantity and abv%,
The formula is: Total ABV % of a cocktail = sum(abv% * quantity)/sum(quantity)
So, we select this from abv ratings and join it on ingredients and group by cocktail name, and insted of WHERE, I used HAVING total_abv % greater than 0 as they are alcoholic cocktails.
So it becomes a rather intuitive query than the other complicated nested queries.
*/
SELECT 
...
/*List all non-alcoholic ones too.
Explanation: To display all the non-alchoholic cocktails, I sorted out the total alcoholic content of a particular cocktail from its ingredients' quantity and abv%,
The formula is: Total ABV % of a cocktail = sum(abv% * quantity)/sum(quantity)
So, we select this from abv ratings and join it on ingredients and group by cocktail name, and insted of WHERE, I used HAVING total_abv % equal to 0 as they are non-alcoholic cocktails.
So it becomes a rather intuitive query than the other complicated nested queries.
*/
SELECT 
    ingredients.cocktail_name AS alcoholic_cocktails,
...
/* Explanation: This is another way to select the non-alcoholic cocktails using a rather complicated way.
Herein, we use NOT IN (SELCT...) nested queries. However it is rather straight forward.*/
SELECT 
    ingredients.cocktail_name AS non_alcoholic_cocktails,
    abv_ratings.`abv(%)`
FROM
    ingredients
        JOIN
    abv_ratings ON ingredients.ingredient_name = abv_ratings.ingredient_name
WHERE
    cocktail_name NOT IN (SELECT 
            ingredients.cocktail_name
        FROM
            ingredients
                JOIN
            abv_ratings ON (abv_ratings.ingredient_name = ingredients.ingredient_name)
        WHERE
            ingredient_type IN ('Alcoholic Drink'))
